...
- [x] 5.0 API Endpoint Discovery and Testing Services
  - [x] 5.1 Create entity `LibreTimeApiEndpoint.java` in `models/` package with all fields from PRD FR-4.2. Use proper JPA annotations, UUID primary key.
  - [x] 5.2 Create entity `LibreTimeApiTestResult.java` in `models/` package with all fields from PRD FR-4.3. Use proper JPA annotations, UUID primary key, foreign key to endpoint.
  - [x] 5.3 Create repositories: `LibreTimeApiEndpointRepository.java` and `LibreTimeApiTestResultRepository.java` with appropriate query methods.
  - [x] 5.4 Create service interface `LibreTimeApiDiscoveryService.java` in `services/` package with methods: discoverEndpoints(), getDiscoveredEndpoints(), updateEndpointStatus().
  - [x] 5.5 Create service implementation `LibreTimeApiDiscoveryServiceImpl.java` in `services/impl/` package. Implement endpoint discovery by testing common REST patterns (GET /api/v2/{resource}, POST /api/v2/{resource}, etc.).
  - [x] 5.6 Implement discovery logic - test standard CRUD endpoints for resource types (files, playlists, smart-blocks, shows, etc.), document HTTP methods, identify URL patterns.
  - [x] 5.7 Create service interface `LibreTimeApiTestingService.java` in `services/` package with methods: testConnection(), testAuthentication(), testEndpoint(), runAllTests(), generateTestReport(), generateBugReport().
  - [x] 5.8 Create service implementation `LibreTimeApiTestingServiceImpl.java` in `services/impl/` package. Implement testing for connectivity, authentication, CRUD operations, error scenarios, edge cases.
  - [x] 5.9 Implement test result storage - save test results to database, categorize endpoints (working, broken, missing, unknown), record detailed test data.
  - [x] 5.10 Implement test report generation - generate reports in JSON, Markdown, and HTML formats. Include summary statistics, detailed results, response times.
  - [x] 5.11 Implement bug report generation - create detailed bug reports for broken endpoints with expected/actual behavior, request/response examples, steps to reproduce, severity levels.
  - [x] 5.12 Create DTOs: `ApiEndpointResponseDTO.java`, `ApiTestResultResponseDTO.java`, `ApiTestSummaryResponseDTO.java`, `BugReportResponseDTO.java` in `dto/` package.
...
- [x] 7.0 Sync Status and History Tracking
  - [x] 7.1 Create entity `LibreTimeSyncHistory.java` in `models/` package with all fields from PRD FR-4.5. Use proper JPA annotations, UUID primary key.
  - [x] 7.2 Create repository `LibreTimeSyncHistoryRepository.java` with query methods for finding by sync type, by status, by date range, by user.
  - [x] 7.3 Create service interface `LibreTimeSyncHistoryService.java` in `services/` package with methods: createSyncHistory(), completeSyncHistory(), getSyncHistory(), getSyncHistoryByDateRange(), getSyncHistoryByType(), getSyncHistoryByUser(), getSyncStatistics().
  - [x] 7.4 Create service implementation `LibreTimeSyncHistoryServiceImpl.java` in `services/impl/` package. Implement sync status tracking, history maintenance, conflict detection.
  - [x] 7.5 Implement sync status tracking - track status for each file (pending, syncing, synced, failed, conflict), maintain timestamps, update status in real-time.
  - [x] 7.6 Implement sync history - record all sync operations with type, status, item counts, timestamps, error summaries, initiated by user.
  - [x] 7.7 Implement conflict detection - compare file hashes, detect when same file modified in both systems, flag conflicts for resolution.
  - [x] 7.8 Implement sync statistics - calculate total files, synced count, failed count, pending count, success rate.
  - [x] 7.9 Create DTOs: `SyncStatusResponseDTO.java`, `SyncHistoryResponseDTO.java`, `SyncStatisticsResponseDTO.java` in `dto/` package.
  - [x] 7.10 Integrate sync status tracking into file sync service - update status after each operation, record history entries.

- [x] 8.0 REST API Controllers
  - [x] 8.1 Create controller `LibreTimeIntegrationController.java` in `controllers/` package. Add @RestController, @RequestMapping("/api/libretime/integration"), Swagger annotations.
  - [x] 8.2 Implement GET endpoint to retrieve integration configuration. Add @PreAuthorize with LIBRETIME_INTEGRATION_VIEW permission, return masked sensitive data.
  - [x] 8.3 Implement PUT endpoint to update integration configuration. Add @PreAuthorize with LIBRETIME_INTEGRATION_CONFIGURE permission, validate input, encrypt sensitive fields.
  - [x] 8.4 Implement POST endpoint to test connection. Add @PreAuthorize with LIBRETIME_INTEGRATION_VIEW permission, return connection test results.
  - [x] 8.5 Create controller `LibreTimeFileSyncController.java` in `controllers/` package. Add @RestController, @RequestMapping("/api/libretime/files"), Swagger annotations.
  - [x] 8.6 Implement POST endpoint for file upload. Add @PreAuthorize with LIBRETIME_INTEGRATION_SYNC permission, accept multipart/form-data, return upload results.
  - [x] 8.7 Implement GET endpoint for file download. Add @PreAuthorize with LIBRETIME_INTEGRATION_VIEW permission, return file and metadata.
  - [x] 8.8 Implement GET endpoint for file listing. Add @PreAuthorize with LIBRETIME_INTEGRATION_VIEW permission, support pagination, filtering, sorting.
  - [x] 8.9 Implement POST endpoint for file querying. Add @PreAuthorize with LIBRETIME_INTEGRATION_VIEW permission, accept query criteria, return matching files.
  - [x] 8.10 Implement POST endpoint for manual sync trigger. Add @PreAuthorize with LIBRETIME_INTEGRATION_SYNC permission, trigger sync operation, return sync status.
  - [x] 8.11 Create controller `LibreTimeApiTestingController.java` in `controllers/` package. Add @RestController, @RequestMapping("/api/libretime/testing"), Swagger annotations.
  - [x] 8.12 Implement POST endpoint to discover endpoints. Add @PreAuthorize with LIBRETIME_INTEGRATION_TEST permission, trigger discovery, return discovered endpoints.
  - [x] 8.13 Implement POST endpoint to run API tests. Add @PreAuthorize with LIBRETIME_INTEGRATION_TEST permission, run tests, return test results.
  - [x] 8.14 Implement GET endpoint to get test results. Add @PreAuthorize with LIBRETIME_INTEGRATION_VIEW permission, return test results with filtering options.
  - [x] 8.15 Implement GET endpoint to get test summary. Add @PreAuthorize with LIBRETIME_INTEGRATION_VIEW permission, return summary statistics.
  - [x] 8.16 Implement GET endpoint to get bug reports. Add @PreAuthorize with LIBRETIME_INTEGRATION_EXPORT_LOGS permission, return bug reports for broken endpoints.
  - [x] 8.17 Implement GET endpoint to export test documentation. Add @PreAuthorize with LIBRETIME_INTEGRATION_EXPORT_LOGS permission, return documentation in requested format (JSON, Markdown, HTML).
  - [x] 8.18 Add GET endpoints for sync status and history in appropriate controllers. Add @PreAuthorize with LIBRETIME_INTEGRATION_VIEW permission.
  - [x] 8.19 Add proper error handling to all controllers - use @ExceptionHandler or GlobalExceptionHandler, return consistent error response format.
  - [x] 8.20 Add comprehensive Swagger/OpenAPI annotations to all endpoints - @Operation, @ApiResponse, parameter descriptions.

- [x] 9.0 Settings UI Implementation
  - [x] 9.1 Create HTML file `libretime-integration-settings.html` in `src/main/resources/static/` directory. Use semantic HTML5 elements, proper structure.
  - [x] 9.2 Implement Connection Settings section - API Base URL input, JWT Token input (masked display), Test Connection button, connection status indicator. Add proper labels, ARIA attributes, keyboard navigation.
  - [x] 9.3 Implement Synchronization Settings section - auto-sync toggle, sync frequency dropdown, sync direction radio buttons, conflict resolution dropdown, manual sync button, last sync timestamp display. Ensure accessibility.
  - [x] 9.4 Implement File Settings section - supported formats checklist, max file size input, storage path input, metadata sync toggle. Add validation, help text.
  - [x] 9.5 Implement Log Generation Settings section - auto-export toggle, validation level dropdown, conflict resolution dropdown, preview toggle, scheduling options. Ensure proper form structure.
  - [x] 9.6 Implement Webhook Settings section - webhook URL input, webhook secret input (masked), enable/disable toggle, test webhook button, delivery status display. Add security considerations.
  - [x] 9.7 Implement API Testing & Documentation section - Discover Endpoints button, Run Tests button, Test Results table/view, Export Documentation button, Bug Report display. Make interactive and accessible.
  - [x] 9.8 Implement Sync Status & History section - current sync status dashboard, sync history table, file sync status list, error log viewer. Add pagination, filtering, sorting.
  - [x] 9.9 Create CSS file `libretime-integration.css` in `src/main/resources/static/css/` directory. Implement responsive design (mobile, tablet, desktop), proper color contrast (WCAG 2.1 Level AA), focus indicators.
  - [x] 9.10 Create JavaScript file `libretime-integration.js` in `src/main/resources/static/js/` directory. Implement API calls to backend endpoints, form validation, dynamic UI updates, error handling.
  - [x] 9.11 Implement form validation in JavaScript - validate URL format, validate file size limits, validate required fields, show clear error messages.
  - [x] 9.12 Implement API integration in JavaScript - fetch integration config, update config, test connection, trigger sync, discover endpoints, run tests, export documentation.
  - [x] 9.13 Implement real-time status updates - poll for sync status updates, update UI dynamically, show loading states, handle errors gracefully.
  - [x] 9.14 Ensure keyboard navigation - all interactive elements keyboard accessible, visible focus indicators, logical tab order, skip links.
  - [x] 9.15 Ensure screen reader compatibility - proper ARIA labels, ARIA describedby for help text, ARIA invalid for errors, semantic HTML structure.
  - [x] 9.16 Test responsive design - verify UI works on mobile (320px+), tablet (768px+), desktop (1024px+). Test on multiple browsers.

- [x] 10.0 Role-Based Access Control and Permissions
  - [x] 10.1 Review existing permission system in `PermissionLevel.java`, `ModuleType.java`, `ActionType.java` to understand current structure.
  - [x] 10.2 Add new permission constants to appropriate enum/class: LIBRETIME_INTEGRATION_VIEW, LIBRETIME_INTEGRATION_CONFIGURE, LIBRETIME_INTEGRATION_TEST, LIBRETIME_INTEGRATION_SYNC, LIBRETIME_INTEGRATION_EXPORT_LOGS.
  - [x] 10.3 Update `LibreLogPermissionEvaluator.java` to handle new LibreTime integration permissions if needed.
  - [x] 10.4 Assign default permissions to user roles - Administrators get all permissions, Operators get VIEW and SYNC, Viewers get VIEW only. Update role initialization or configuration.
  - [x] 10.5 Update all controller endpoints to use @PreAuthorize with appropriate permissions (already added in task 8.0, verify completeness).
  - [x] 10.6 Update UI JavaScript to check permissions before displaying/enabling features - hide/disable features based on user permissions.
  - [x] 10.7 Add permission checks in service layer if needed - verify permissions before performing sensitive operations.

- [x] 11.0 Error Handling and Logging Enhancements
  - [x] 11.1 Review existing `GlobalExceptionHandler.java` and ensure it handles all new exception types from LibreTime integration.
  - [x] 11.2 Create custom exceptions if needed: `LibreTimeConnectionException.java`, `LibreTimeApiException.java`, `FileSyncException.java` in `exceptions/` package.
  - [x] 11.3 Implement error handling in `LibreTimeHttpClient` - handle network errors, authentication errors (401, 403), validation errors (400), server errors (500, 502, 503) with appropriate exception types.
  - [x] 11.4 Implement error handling in file sync service - handle upload failures, download failures, file not found, permission denied, with detailed error messages.
  - [x] 11.5 Implement error handling in API testing service - handle test failures gracefully, capture error details, provide actionable error messages.
  - [x] 11.6 Add comprehensive logging to all services - log API requests/responses, sync operations, errors with appropriate log levels (INFO, WARN, ERROR, DEBUG).
  - [x] 11.7 Ensure sensitive information masking in logs - mask JWT tokens, passwords, webhook secrets in all log statements.
  - [x] 11.8 Implement user-friendly error messages in controllers - convert technical errors to actionable messages for end users, log technical details separately.
  - [x] 11.9 Add error response DTO if not exists - ensure all error responses follow consistent format from PRD design considerations.

- [x] 12.0 Testing (Unit, Integration, UI)
  - [x] 12.1 Create unit tests for all new enum classes following existing pattern (e.g., `UserStatusTest.java`). Test values(), valueOf(), ensure 80%+ coverage.
  - [x] 12.2 Create unit tests for `LibreTimeIntegrationConfigServiceImpl` - test getConfig(), saveConfig(), updateConfig(), testConnection(), encryption/decryption. Use Mockito for dependencies, aim for 80%+ coverage.
  - [x] 12.3 Create unit tests for `LibreTimeFileSyncServiceImpl` - test uploadFile(), downloadFile(), listFiles(), queryFiles(). Mock LibreTimeHttpClient, test error scenarios, aim for 80%+ coverage.
  - [x] 12.4 Create unit tests for `LibreTimeApiDiscoveryServiceImpl` - test discoverEndpoints(), getDiscoveredEndpoints(). Mock HTTP client, test various endpoint patterns, aim for 80%+ coverage.
  - [x] 12.5 Create unit tests for `LibreTimeApiTestingServiceImpl` - test testConnection(), testEndpoint(), runAllTests(), generateTestReport(), generateBugReport(). Mock HTTP client, test all test types, aim for 80%+ coverage.
  - [x] 12.6 Create unit tests for `LibreTimeSyncHistoryServiceImpl` - test createSyncHistory(), completeSyncHistory(), getSyncHistory(), getSyncStatistics(). Mock repositories, test various scenarios, aim for 80%+ coverage.
  ...
  - [x] 12.9 Create integration test `LibreTimeIntegrationIT.java` in `integration/` package. Use @SpringBootTest, test end-to-end workflows: configure → test connection → upload file → verify sync status. Use Testcontainers if needed for external dependencies.
  - [x] 12.10 Create integration test for API discovery and testing - test actual discovery against test LibreTime instance (if available), or use mocks. Test full discovery and testing workflow.
  ...
  - [x] 12.15 Run full test suite with `mvn clean install` - verify all tests pass, verify JaCoCo coverage meets 80% minimum requirement, fix any failing tests or coverage gaps.
